'''This is the main driver program of the Checkers Game'''
import sys
import time
import getopt
...
def doMove(board, move):
	# Performs all the move actions from the move list	
...
def canMoveToPosition(board, x1, y1, x2, y2):
	# Checks whether (x1,y1) can move to (x2,y2) in one move (plain or capture)
...
def isLegalMove(board, move, color):
	# Checks whether move (a list) is a legal move in the board for <color> piece
...
def countPieces(board, color):
	# Returns number of <color> pieces (man or king) there are
...
def serialToGrid(serial):
	# Given a piece's serial 1-32 it returns the board grid position (0,0)~(7,7)
	
	return ((serial-1)//4, 2*((serial-1)%4)+1-((serial-1)//4)%2)


def newBoard():
	# Creates a new board, 2D array of characters
...
def playGame(p1, p2, verbose, t = 150):
	# Takes as input two functions/players p1 and p2 (each of which
	# calculates a next move given a board and player color),
	# and returns a tuple containing the final board, 
	# pieces left for red,
